...
const AllRecipes = () => {
    ...
    const getAllRecipes = async () => {
        try {
            const res = await api.get("/api/recipes/all-recipes/");
        ...
        api.delete(`/api/recipes/delete/${recipeToDelete}/`)
            .then((res) => {
                if (res.status === 204 || res.status === 200) {
                    ...
                    setAllRecipes(prevRecipes => prevRecipes.filter(recipe => recipe.id !== recipeToDelete));
                } else {
            ...
            {loading ? (
                ...
                allRecipes.length > 0 ? (
                    ...
                    <Typography variant="h5" component="p" align="center" color='text.secondary' sx={{ mt: 4 }}>
                        No recipes found. Please login or register to add some!
                    </Typography>
                )
            }
